...
class PyGameDisplayer(Displayer):
    def __init__(self, width, height):
        super().__init__(width, height)
        pygame.init()
        self.screen = pygame.display.set_mode((width * FIELD_BASE_WIDTH, height * FIELD_BASE_WIDTH))
    ...
    def display(self, grid):
        ...
        for i in range(self.height):
            for j in range(self.width):
                start_x, start_y = i * FIELD_BASE_WIDTH, j * FIELD_BASE_WIDTH
                mid_x, mid_y = start_x + FIELD_BASE_WIDTH // 2, start_y + FIELD_BASE_WIDTH // 2
                field_start_x = start_x + FIELD_PADDING
                field_start_y = start_y + FIELD_PADDING
    ...
    def draw_field(self, x, y):
        pygame.draw.rect(self.screen, BLACK, (x, y, FIELD_WIDTH, FIELD_WIDTH), 1)
    ...
    def draw_goal(self, x, y):
        ...
        text = font.render("$", 2, GREEN)
        self.screen.blit(text, (x - 15, y - 25, 50, 50))

    def draw_wall(self, x, y):
        r = FIELD_WIDTH // 2
        pygame.draw.lines(self.screen, OBSTACLE_COLOUR, False, [(x - r, y - 1), (x + r - 1, y - 1)], 20)
        pygame.draw.lines(self.screen, OBSTACLE_COLOUR, False, [(x - 1, y - r), (x - 1, y + r - 1)], 20)
    ...
    def draw_figure(self, x_mid, y_mid):
        ...
        pygame.draw.circle(self.screen, BLACK, (x_mid, head_y), 8, 1)

        pygame.draw.lines(self.screen, OBSTACLE_COLOUR, False,
                          [(l_hand_x - 5, hand_y + 5), (l_hand_x + 5, hand_y - 5)], 3)
        pygame.draw.lines(self.screen, OBSTACLE_COLOUR, False,
                          [(r_hand_x - 5, hand_y - 5), (r_hand_x + 5, hand_y + 5)], 3)

        pygame.draw.lines(self.screen, OBSTACLE_COLOUR, False, [(l_leg_x + 2, leg_y - 7), (l_leg_x - 2, leg_y + 10)], 3)
        pygame.draw.lines(self.screen, OBSTACLE_COLOUR, False, [(r_leg_x - 2, leg_y - 7), (r_leg_x + 2, leg_y + 10)], 3)

        pygame.draw.rect(self.screen, YELLOW, (body_x, body_y, 12, 24), 0)
